...
function isValidElementType(type) {
  ...
  type === REACT_FRAGMENT_TYPE || type === REACT_CONCURRENT_MODE_TYPE || type === REACT_PROFILER_TYPE || type === REACT_STRICT_MODE_TYPE || type === REACT_SUSPENSE_TYPE || type === REACT_SUSPENSE_LIST_TYPE || typeof type === 'object' && type !== null && (type.$$typeof === REACT_LAZY_TYPE || type.$$typeof === REACT_MEMO_TYPE || type.$$typeof === REACT_PROVIDER_TYPE || type.$$typeof === REACT_CONTEXT_TYPE || type.$$typeof === REACT_FORWARD_REF_TYPE || type.$$typeof === REACT_EVENT_COMPONENT_TYPE);
}
...
function useEvent(eventComponent, props) {
  var dispatcher = resolveDispatcher();
  return dispatcher.useEvent(eventComponent, props || emptyObject$1);
}
...
function flushWork(hasTimeRemaining, initialTime) {
  ...
  try {
    if (!hasTimeRemaining) {
      ...
      // a boolean argument?
      while (firstTask !== null && firstTask.expirationTime <= currentTime && !(enableSchedulerDebugging$1 && isSchedulerPaused)) {
      ...
      if (firstTask !== null) {
        do {
          ...
          currentTime = getCurrentTime();
          advanceTimers(currentTime);
        } while (firstTask !== null && !shouldYieldToHost() && !(enableSchedulerDebugging$1 && isSchedulerPaused));
      }
    }
...
function createEventComponent(responder) {
...
if (enableFlareAPI) {
  React.unstable_createEvent = createEventComponent;
